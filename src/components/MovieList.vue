...
import { addWishlist, getMovie, movieSearch } from "../script/script";
  ...
  <div class="container">
    <div class="category col-md-3 col-lg-2 border-end border-secondary" style="min-height: 100vh">
      <select class="LangSelect" style="width: 150px" v-model="langType" placeholder="언어">
        ...
        <option :value="item.iso_3166_1" v-for="item in langueageTypes" :key="item.iso_3166_1">
          {{ item.english_name }}
        </option>
      </select>
    ...
    <div class="items">
      ...
      <div class="movies">
        ...
        <div v-for="item in movieData.results" :key="item.id" v-else>
          <div @click="moveToDetail(item.id)">
          ...
          <div>
            <button class="w-100" @click="addWishlist(item.id)">찜하기</button>
          </div>
        </div>
      </div>
